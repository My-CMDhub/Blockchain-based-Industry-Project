/**
 * Secret Manager utilities that can use either local file storage or Google Cloud Secret Manager
...
// Determine which backend to use (local or gcp)
const SECRETS_BACKEND = process.env.SECRETS_BACKEND || 'local';

// Import the GCP secrets manager if needed
let gcpSecretsManager;
if (SECRETS_BACKEND === 'gcp') {
  try {
    gcpSecretsManager = require('./gcpSecretsManager');
    console.log('GCP Secret Manager backend initialized and ready');
  } catch (err) {
    console.error('Failed to initialize GCP Secret Manager backend:', err.message);
    console.warn('Falling back to local file-based secret storage');
  }
}

// Log which backend is being used
console.log(`Secret Manager backend: ${SECRETS_BACKEND.toUpperCase()}`);
...
// Initialize with message indicating active mode
if (SECRETS_BACKEND === 'local') {
  console.log('Local file-based secrets management enabled. Using local file storage only.');
} else if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
  console.log('Google Cloud Secret Manager enabled. Secrets will be stored in GCP Secret Manager.');
} else {
  console.log('Invalid or missing secret backend configuration. Falling back to local file storage.');
}

// Ensure secure directory exists for local backend
if (SECRETS_BACKEND === 'local') {
  try {
...
/**
 * Get a secret from the configured backend (GCP or local)
 * @param {string} secretName - Name of the secret
 * @returns {Promise<Object>} - The secret value as an object
 */
async function getSecret(secretName) {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.getSecret(secretName);
  }
  
  // Otherwise use local file-based implementation
...
/**
 * Update a secret in the configured backend (GCP or local)
 * @param {string} secretName - Name of the secret
...
async function updateSecret(secretName, secretValue) {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.updateSecret(secretName, secretValue);
  }
...
async function getWalletMetadata() {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.getWalletMetadata();
  }
...
async function updateWalletMetadata(metadata) {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.updateWalletMetadata(metadata);
  }
...
async function getSensitiveKeys() {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.getSensitiveKeys();
  }
...
async function updateSensitiveKeys(keys) {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.updateSensitiveKeys(keys);
  }
...
async function getPrivateKey() {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.getPrivateKey();
  }
...
async function updatePrivateKey(privateKey) {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.updatePrivateKey(privateKey);
  }
...
async function migrateLegacyKeysIfNeeded() {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.migrateLegacyKeysIfNeeded();
  }
...
async function getStoredKeys() {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.getStoredKeys();
  }
...
async function updateStoredKeys(keys) {
  // Use GCP backend if configured and available
  if (SECRETS_BACKEND === 'gcp' && gcpSecretsManager) {
    return gcpSecretsManager.updateStoredKeys(keys);
  }
...
/**
 * Get information about the current secrets backend
 * @returns {object} Information about the secrets backend
 */
function getBackendInfo() {
  return {
    backend: SECRETS_BACKEND,
    useGcp: SECRETS_BACKEND === 'gcp' && gcpSecretsManager !== undefined,
    localPath: SECURE_DIR,
